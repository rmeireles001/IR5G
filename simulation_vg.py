...
def reset_blade(anti_affinity_list, vgreedy):
    for s in slices:
        s.subscribers = 0
        for k in s.vnfs:
...
def simulation_add_subscriber(new_subs, results, mapping, log, vector):
        ...
        if slices[slc].subscribers > (vnfs.num_vcpus()/(vnfs.scale_factor*pow(10,-4))):
            ...
            for dd in hyper.datacenter:
                for bb in hyper.datacenter[dd]:
                    log.write(f'{dd} {bb} {hyper.datacenter[dd][bb].resource} {hyper.datacenter[dd][bb].resources_clock()}\n')
            if(flag_affinity==1):
                flag_affinity = vnfs.add_module_vgreedy_af(i,slices[slc].name+"#"+vnfs.name, vgreedy)
                if(flag_affinity==0):
                    vector[1] = flag_affinity
                    log.write("Affinity rule break\n")
                    log.write(f'{slices[slc].name}_{key}#{i} {slices[slc].subscribers}  {(vnfs.num_vcpus()/(vnfs.scale_factor*pow(10,-4)))}\n')
                    slices[slc].subscribers = slices[slc].subscribers - 1
                    tot_subscribers = slices[0].subscribers + slices[1].subscribers
                    results.write(f'{tot_subscribers}\t')

                    for s in slices:
                        results.write(f'{s.subscribers}\t')
                        for k in s.vnfs:
                            results.write(f'{len(s.vnfs[k].modules)}\t')
                    ...
                    results.write(f'{tot_cpu - used_res["cpu"]}\t{tot_ram - used_res["ram"]}\t{tot_ssd - used_res["ssd"]}\t{delta_cpu}\t{delta_ram}\t{delta_ssd}\t')
                    ...
                    for d in hyper.datacenter:
                        for b in hyper.datacenter[d]:
                            ...
                            mapping.write(f'{b}\t{hyper.datacenter[d][b].resource["cpu"]}\t{hyper.datacenter[d][b].resource["ram"]}\t{hyper.datacenter[d][b].resource["ssd"]}\t{used_resources["cpu"]}\t{used_resources["ram"]}\t{used_resources["ssd"]}\t')
                    for s2 in slices:
                        for key2, vnfs2 in s2.vnfs.items():
                            for var in vnfs2.modules:
                                mapping.write(vnfs2.modules[var].guestname+"\t"+vnfs2.modules[var].hostname+"\t")
                    results.write(slices[slc].name+"_"+vnfs.name+"#"+str(i)+"\t")
                    slices[slc].subscribers = slices[slc].subscribers + 1
                else:
                    log.write(f'{slices[slc].name}_{key}#{i} - {vnfs.modules[slices[slc].name+"#"+vnfs.name+str(i)].hostname}\n')
                    vector[0] = 1
                    return
            if(flag_affinity==0):
                flag_affinity_resources = vnfs.add_module_vgreedy_naf(i,s.name+"#"+vnfs.name, vgreedy)
                if(flag_affinity_resources==0):
                    log.write("No more resources\n")
                    log.write(f'{slices[slc].name}_{key}#{i} {slices[slc].subscribers}  {(vnfs.num_vcpus()/(vnfs.scale_factor*pow(10,-4)))}\n')
                    slices[slc].subscribers = slices[slc].subscribers - 1
                    tot_subscribers = slices[0].subscribers + slices[1].subscribers
                    results.write(f'{tot_subscribers}\t')
